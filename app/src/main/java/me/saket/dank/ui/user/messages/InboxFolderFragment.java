...
public class InboxFolderFragment extends DankFragment {
  ...
  interface Callbacks {
    BehaviorRelay<List<Message>> messageStream(InboxFolder folder);
  ...
  public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
    ...
    unsubscribeOnDestroy(((Callbacks) getActivity()).messageStream(folder)
  ...
  private void startInfiniteScroll() {
    InfiniteScroller.ViewStateCallbacks<Message> viewStateCallbacks = new InfiniteScroller.ViewStateCallbacks<Message>() {
      ...
      public void updateItemsDataset(List<Message> items) {
        // We avoid updating the data-set here. Instead, InboxActivity stores all fetched items
        // locally and we subscribe to its stream in onViewCreated(). This way, we can avoid
        // force-refreshing messages everytime this fragment gets recreated.
      }
      ...
      public void setErrorOnMoreLoadVisible(@Nullable Throwable error, boolean errorVisible) {
        ...
        if (errorVisible) {
          ...
          loadMoreErrorSnackbar.setAction(R.string.common_error_retry, v -> startInfiniteScroll());
    ...
    boolean firstLoadDone = ((Callbacks) getActivity()).messageStream(folder).hasValue();
    scrollListener.setEmitInitialEvent(!firstLoadDone);
